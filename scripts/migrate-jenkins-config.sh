...
echo "🔄 Starting Jenkins to GitHub Actions migration for project: $PROJECT_ROOT"
...
extract_value() {
    ...
    if [ -z "$value" ] && [ -n "$default_value" ]; then
        echo "⚠️  Warning: Key '$key' not found in $file. Using default: '$default_value'"
        value="$default_value"
    elif [ -z "$value" ]; then
        echo "⚠️  Warning: Key '$key' not found in $file. No default set."
    fi
    echo "$value"
}
...
detect_quality_tools() {
    local tools=()
    echo "🔎 Detecting quality tools..." >&2
...
detect_tool_binaries() {
    ...
    echo "🔎 Checking for custom tool binary paths..." >&2
...
if [ ${#JENKINS_FILES[@]} -eq 0 ]; then
    echo "❌ No Jenkinsfile-project-* files found in $PROJECT_ROOT. Aborting."
    exit 1
fi
echo "✅ Found ${#JENKINS_FILES[@]} Jenkins configuration file(s)."

# 2. Extract common settings from the first file found
FIRST_FILE="${JENKINS_FILES[0]}"
echo "📄 Using '$FIRST_FILE' for common settings."
...
echo "📝 Generating pipeline configuration file at '$OUTPUT_FILE'..."
...
# --- Completion --- 
echo "✅ Generated pipeline configuration: $OUTPUT_FILE"
echo ""
